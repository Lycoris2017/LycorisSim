...
void MomentumResolution(){
  //	TFile *f1  = TFile::Open("/afs/desy.de/group/flc/pool/fmueller/tpcsimulation/run_B1_E0.root","Update");
  ...
  c1->cd(2);
  MomentumResolutionSmearing->Fit("CrystalBallReso","R");
  ...
  TFile* outfile = TFile::Open("MomentumResolution.root","recreate");
  c1->Write();
  c2->Write();
  c3->Write();
  c4->Write();
